...
namespace mmaurice\PaymentLinksModule;
...
<h1>
    <i class="fa fa-ruble-sign"></i> Генератор ссылок на оплату
</h1>
        ...
        <div class="tab-pane" id="documentPane">
            <script type="text/javascript">
                var tpSettings = new WebFXTabPane(document.getElementById('documentPane'),
                    <?= ($modx->getConfig('remember_last_tab') == 1 ? 'true' : 'false') ?>);
            </script>
...
<script>
    let apiUrl =
        '<?= str_replace(['/', '\\'], '/', str_replace(realpath(MODX_BASE_PATH), '', realpath(dirname(__FILE__)))) . '/api.php'; ?>';
    let linkPrefix = '<?= (!is_null($pageHandler) ? '/' . trim(is_numeric($pageHandler) ? $modx->makeUrl($pageHandler) : $pageHandler, '/') : ''); ?>';
</script>
...
<script src="<?= $modulePath . '/assets/scripts/tabs/generatorTabScripts.class.js'; ?>?v=<?= time(); ?>"
